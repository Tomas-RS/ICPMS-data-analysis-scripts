...
Intensities <- read.csv("Sample_run_200814.csv", stringsAsFactors = FALSE)
View(Intensities)
Sample_prep <- read.csv("Sample_digest_200814.csv", stringsAsFactors = FALSE)
view(Sample_prep)
...
sample_names <- sample_names[!sample_names %in% c("0.09918 ppb metal standard", 
                                                  "0.9714 ppb metal standard", 
                                                  "4.917 ppb metal standard", 
                                                  "9.771 ppb metal standard", 
                                                  "48.76 ppb metal standard", 
                                                  "96.89 ppb metal standard", 
                                                  "196.7 ppb metal standard", 
                                                  "Blank", 
                                                  "1 ppb standard")]
...
for (analyzedMetal in Metals_names) {
  ...
  #This step has been modified so that a plot is produced but no drift adjustment takes place
  ...
  #THE LINE BELOW HAS BEEN MODIFIED SO THAT NO DRIFT ADJUSTMENT IS APPLIED
  df_cps_metal$adjust_drift <- df_cps_metal$Scadjust
  ...
  metal_results <- data.frame(Sample_name = df_cps_metal_samples$Sample_name, 
                              metal = analyzedMetal, 
                              Concentration_in_original_sample = df_cps_metal_samples$Sample_ppb, 
                              RSD = df_cps_metal_samples$SD_ppb, 
                              Concentration_in_measured_sample = df_cps_metal_samples$ppb, 
                              Detection_limit = metal_detectionlimit)
  

  
  #STEP EIGHT: Make plots and save data to file
  
  plot <- ggplot(metal_results, aes(x = Sample_name, y = Concentration_in_original_sample)) +  
    geom_point() + 
    geom_errorbar(aes(ymin = Concentration_in_original_sample - RSD,
                      ymax = Concentration_in_original_sample + RSD),
                  width = 0.5,
  ...
  plot_log10 <- ggplot(metal_results, aes(x = Sample_name, y = Concentration_in_original_sample)) + 
    geom_point() + 
    geom_errorbar(aes(ymin = Concentration_in_original_sample - RSD,
                      ymax = Concentration_in_original_sample + RSD),
                  width = 0.5,
  ...
  colnames(metal_results) <- c("Sample", 
                               "metal",
                               "Concentration in original sample (ppb)", 
                               "RSD (± ppb)", 
                               "Concentration in measured sample (ppb)",
                               "Detection limit (ppb)")
