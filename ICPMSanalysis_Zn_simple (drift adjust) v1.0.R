#Simple script for ICP-MS data analysis of zinc (single metal, with drift adjust) 
...
#This gives new column (Z03_Zn_driftadjust) in Zinc_analysis
Zinc_driftplot <- lm(Zinc_standards$Z02_Scadjust ~ Zinc_standards$Sequence_position)
Zinc_analysis$Z03_Zn_driftadjust <- Zinc_analysis$Z02_Scadjust - coef(Zinc_driftplot)[2] * 
  (Zinc_analysis$Sequence_position - Zinc_standards[1,1])
...
plot(Zinc_blanks$Sequence_position, Zinc_blanks$Z03_Zn_driftadjust, 
...
Zinc_analysis$Z04_Zn_blankadjust <- Zinc_analysis$Z03_Zn_driftadjust - mean(Zinc_blanks[["Z03_Zn_driftadjust"]])
...
Zinc_detectionlimit_ppb <- (3 * sd(Zinc_blanks[["Z03_Zn_driftadjust"]])) / coef(Zinc_stdplot)[2]
